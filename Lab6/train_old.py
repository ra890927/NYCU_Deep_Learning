...
one_minus_alpha_cumprod_t_minus_1 = torch.cat(
    (torch.tensor(1).unsqueeze(0), (1 - alpha_cumprod)[:-1]))
one_minus_alpha_cumprod = (1 - alpha_cumprod)
sqrt_variance = torch.sqrt(
    (beta * (one_minus_alpha_cumprod_t_minus_1/one_minus_alpha_cumprod)))
...
def train(args, model, device, train_loader, optimizer, epoch, lr_scheduler, accelerator):
    ...
    for batch_idx, (data, cond) in enumerate(train_loader):
        ...
        '''
        ''''''
        output = model(sample=xt.to(args.device), timestep=rand_t.to(
            args.device), class_labels=cond.to(torch.float32).to(args.device))
...
def main():
    ...
    # manipulate data
    train_loader = torch.utils.data.DataLoader(iclevrLoader(
        root="./dataset/", mode="train"), **train_kwargs, shuffle=True)
    print(len(train_loader.dataset))
    test_loader = torch.utils.data.DataLoader(iclevrLoader(
        root="./dataset/", mode="test"), **test_kwargs, shuffle=False)
    test_loader_new = torch.utils.data.DataLoader(iclevrLoader(
        root="./dataset/", mode="new_test"), **test_kwargs, shuffle=False)
    # create model
    model = UNet2DModel(
        sample_size=64,
        in_channels=3,
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, model, device, train_loader,
              optimizer, epoch, lr_scheduler, accelerator)
        if args.save_model and (epoch % 1) == 0:
            if args.save_model and (epoch % 5) == 0:
                model.save_pretrained("./local-unet"+"epoch_" +
                                      str(epoch), variant="non_ema")
            print("==test.json==")
