...
class DDPG:
    ...
    def _update_behavior_network(self, gamma):
        ...
        ## update critic ##
        # critic loss
        predict_targets = critic_net(state, action)
        with torch.no_grad():
            next_action = target_actor_net(next_state)
            label_next = target_critic_net(next_state, next_action)
            labels = reward + gamma * label_next * (1 - done)

        criterion = nn.MSELoss()
        critic_loss = criterion(predict_targets, labels)
        # optimize critic
        actor_net.zero_grad()
        ...
        ## update actor ##
        # actor loss
        predict_action = actor_net(state)
        actor_loss = -critic_net(state, predict_action).mean()
